...
int mb_slave_init(void)
{
  ...
  */
  ...
  if (ctx == NULL) {
    LOGE("Unable to allocate libmodbus context\n");
    return -1;
  }

  mb_mapping = modbus_mapping_new(0, 0, MB_SLAVE_REGS_NB, 0);
  if (mb_mapping == NULL) {
    LOGE("Failed to allocate the mapping: %s\n", modbus_strerror(errno));
    modbus_free(ctx);
    return -1;
  }
  ...
  if (server_socket == -1) {
    LOGE("Unable to listen TCP connection\n");
    modbus_free(ctx);
    return -1;
  }
...
int mb_slave_check(int usec /*= 1000000*/)
{
  ...
  for (master_socket = 0; master_socket <= fdmax; master_socket++) {
    ...
    if (master_socket == server_socket) {  // Client asking a new connection
      ...
      if (newfd == -1) {
        LOGE("Server accept() error\n");
        perror("Server accept() error");
      } else {
        FD_SET(newfd, &refset);
        if (newfd > fdmax)
          fdmax = newfd;  // Keep track of the maximum
        LOGI("New connection from %s:%d on socket %d\n",
      ...
      if (rc > 0)
        ...
        LOGI("Connection closed on socket %d\n", master_socket);
        close(master_socket);
