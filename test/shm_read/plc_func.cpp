...
int plc_show1() {
  ...
  for (int i = 0; i < nb_plcs; ++i) { // Cycle for PLCs
    ...
    for (int j = 0; j < PLCvec[i].reg_qty; ++j) // Cycle for REGs
...
void plc_print_details(int i) {
  cout << setw(5) << left << PLCvec[i].dev_name << "  " << setw(5) << left
       << PLCvec[i].reg_qty << "  " << setw(15) << left << PLCvec[i].ip_addr
       << "  " << PLCvec[i].mb_interval_ms << "  " << PLCvec[i].mb_timeout_us
       << endl;
  return;
}

void plc_print_reg_details(int i, int j) {
  cout << "       " << setw(9) << left << PLCvec[i].regs[j].ch_name << setw(3)
       << right << PLCvec[i].regs[j].raddr << setw(7) << right
       << PLCvec[i].regs[j].rvalue << "  " << left << PLCvec[i].regs[j].ch_mode
       << " " << endl;
  return;
}
